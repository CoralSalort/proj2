...
public class Hand implements Comparable<Object>{
   ...
   */
    ...
    public Card [] findSet(){
        ...
        // Vamos a ir carta por carta
        for (int i = 0; i < hand.size(); i++) {
            // Chequeamos la carta proxima y la que le sigue
            for (int j = i+1; j < hand.size(); j++) {
                if(j + 1 < hand.size()){ // Nos aseguramos que hay una tercera carta
                    if((this.cardRank(i) == this.cardRank(j)) && (this.cardRank(i) == this.cardRank(j+1))){
                        // Si las 3 son iguales, las cogemos las tres
